...
class Tokenizer:
    def __init__(
        ...
        self.tokenize_dict = {
            chave: valor + 4 for chave, valor in self.tokenize_dict.items()
        }
        self.detokenize_dict = {
            int(chave) + 4: valor for chave, valor in self.detokenize_dict.items()
        }

    def tokenize(self, texto):
        tokens = []
        for char in texto:
            if char in self.tokenize_dict:
    ...
    def detokenize(self, tokens):
        ...
        for token in tokens:
            if token == UNK_TOKEN:
                texto += "[UNK]"
            elif token not in [PAD_TOKEN, EOS_TOKEN, BOS_TOKEN]:
