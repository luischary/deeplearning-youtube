...
if __name__ == "__main__":
    ...
    dados = torch.load(
        "modelos_treinados/imdb_classifier_pretrained_200k/modelo_treinado.pt",
        map_location="cpu",
    )
    ...
    final_preds = (np.array(probas) > best_threshold).astype(int)
    # coloca na base para eu verificar depois os erros
    base = test_dataloader.dataset.dados.copy()
    base["pred"] = final_preds.tolist()
    base["modelo"] = "imdb_classifier_pretrained_200k"
    base.to_parquet(
        "data/imdb/predicts/imdb_classifier_pretrained_200k.pq", index=False
    )
