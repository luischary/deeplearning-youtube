...
def _encode_single_text(text, merges):
    ...
    # precisa deixar os especial tokens negativos no comeco mesmo
    for esp_token, esp_token_idx in SPECIAL_TOKENS.items():
        # subtrai o tamanho agora para somar depois no final
        esp_token_idx -= len(SPECIAL_TOKENS)
        sequencia = list(esp_token.encode("utf-8"))
        # substitui na sequencia pelo idx do token especial
        for i in range(len(tokens) - len(sequencia) + 1):
            if tokens[i : i + len(sequencia)] == sequencia:
                tokens = tokens[:i] + [esp_token_idx] + tokens[i + len(sequencia) :]
    ...
    # soma len(tokens_especiais) nos tokens para ajustar os IDs
    tokens = [t + len(SPECIAL_TOKENS) for t in tokens]
    return tokens


SPECIAL_TOKENS = {"<PAD>": 0, "<BOS>": 1, "<EOS>": 2, "<MASK>": 3}


class BPETokenizer:
    def __init__(self, merges_path: str = None, vocab_size: int = None):
        self.merges = {}
        self.vocab = {}
        if merges_path:
            self.load_merges(merges_path)
        if vocab_size and merges_path:
            # usa so os top vocab_size merges mais frequentes
            self.merges = dict(
                list(self.merges.items())[: vocab_size - 256 - len(SPECIAL_TOKENS)]
            )
            # soma len(tokens_especiais) nos merges para ajustar os IDs
            # self.merges = {
            #     (p1 + len(SPECIAL_TOKENS), p2 + len(SPECIAL_TOKENS)): new_id
            #     + len(SPECIAL_TOKENS)
    ...
    def decode(self, ids):
        # subtrai os tokens especiais para ajustar os IDs dos merges
        ids = [i - len(SPECIAL_TOKENS) for i in ids]
        # Concatena os bytes de cada token ID
        tokens_bytes = b"".join(self.vocab[idx] for idx in ids if idx >= 0)
