...
def generate_by_nucleus(
    ...
    for _ in range(len(tokens), max_len):
        ...
        nucleus_mask = cumulative_probas <= top_p

        # Garante que pelo menos o token mais provável esteja disponível
        nucleus_mask[0] = True

        filtered_probas = sorted_probas[nucleus_mask]
        ...
        sampled_idx = torch.multinomial(filtered_probas, num_samples=1).item()
        next_token = filtered_indices[sampled_idx].item()
