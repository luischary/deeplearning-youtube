...
from src.model import CachedDecoderLM, RotaryCachedDecoderLM
...
def generate_by_nucleus(
    ...
    tokens = model.generate(
        tokens_t, SPECIAL_TOKENS["<EOS>"], top_p=top_p, temperature=temperature
    )

    end_mem = torch.cuda.memory_allocated()

    return tokenizer.decode(tokens), len(tokens), end_mem - start_mem
...
if __name__ == "__main__":
    ...
    #     dropout=0.1,
    # )
    # pesos = load_torch_checkpoint(
    #     "modelos_treinados/decoder_wiki_base/last.pt",
    #     map_location="cpu",
    #     weights_only=False,
    # )["modelo"]

    # modelo = RotaryCachedDecoderLM(
    #     vocab_size=10_000,
    #     embed_dim=768,
    #     num_heads=12,
    #     hidden_size=768 * 4,
    ...
    # )
    # pesos = load_torch_checkpoint(
    #     "modelos_treinados/rotary_decoder_wiki_base/last.pt",
    #     map_location="cpu",
    #     weights_only=False,
    # )["modelo"]
    ...
    # )
    # pesos = load_torch_checkpoint(
    #     "modelos_treinados/rotary_gqa_decoder_wiki_base/last.pt",
    #     map_location="cpu",
    #     weights_only=False,
    # )["modelo"]
    # exit()
    ...
    generate_with_effect(
        modelo,
        "A cidade de São Paulo é conhecida por ",
        tokenizer,
        top_p=0.9,
        temperature=0.7,
        max_len=MAX_LEN,
    ...
    for prompt in [
        ...
        for t in range(6, 9):
            ...
            for _ in range(1):
                ...
                # )
                gerado, num_tokens, mem = generate_by_nucleus(
                    modelo,
                    prompt,
                    tokenizer,
                    top_p=0.9,
    ...
    print(
        f"Memória total utilizada por 1000 tokens gerados: {total_mem / total_tokens / (1024 ** 2) * 1000:.2f} MB"
    )
